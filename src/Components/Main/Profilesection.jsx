...
function Profilesection() {
  ...
  return (
    <div className='w-1/4 h-screen bg-blue-100 text-black sticky top-0 z-10  ' >
      ...
      {checkSearchUser === "exist" ? (
        <div onClick={() => navigate(`/searchedUser/${searchedUserId}`)} className={`absolute z-10 bg-white w-80 top-[70px] left-[30px] flex gap-7 items-center p-5 rounded-[50px] transition-opacity duration-300 ${checkSearchUser === "exist" ? "opacity-100" : "opacity-0"} `} >
      ...
      <div className='h-[56vh] bg-white mt-5 m-10 mb-5 relative rounded-2xl overflow-hidden' >
        ...
        <div className=' absolute top-[100px] left-[110px] h-[90px] w-[90px] rounded-[50%] shadow-zinc-600 shadow-lg overflow-hidden '>
          <img className="h-full w-full object-cover object-center" src={`${import.meta.env.VITE_IMAGE_URL}${user?.profile}`} alt="" />
        </div>
        <div className='mt-12 w-50 flex flex-col justify-center items-center gap-2 m-auto ' >
          <h1 className=' font-bold ' >{user?.name}</h1>
          <div>{user?.bio}</div>
        </div>
        <div className=' mt-7 flex border-y-2 w-60 m-auto border-gray-400 ' >
          <div className='mt-2 mb-2 w-50 flex flex-col justify-center items-center gap-2 border-r-2 border-gray-400 ' >
            <h1 className=' font-bold ' >5</h1>
            <div>Follower</div>
          </div>
          <div className='mt-2 mb-2 w-50 flex flex-col justify-center items-center gap-2  ' >
      ...
      <h1 className=' font-bold text-2xl ml-3 ' >People you may know...</h1>   
